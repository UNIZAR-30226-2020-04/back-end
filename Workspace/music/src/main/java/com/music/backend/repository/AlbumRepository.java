...
public interface AlbumRepository extends CrudRepository<Album,keyLista> {
	...
	@Query(value = "SELECT * FROM zzalbum WHERE lista_id = ?1 AND usuario_id = ?2", nativeQuery = true)
	public Album findById(int i, String s);
	@Query(value = "SELECT * FROM zzalbum WHERE usuario_id = ?1", nativeQuery = true)
	public Iterable<Album> getByUser(String s);
	@Query(value = "SELECT * FROM zzalbum WHERE usuario_id = ?1", nativeQuery = true)
	public Album[] listAlbumsUser(String s);
	@Query(value = "SELECT * FROM zzalbum WHERE nombre LIKE %?1%", nativeQuery = true)
