...
public interface CapituloRepository extends CrudRepository<Capitulo,keyCancion> {
	...
	@Query(value = "SELECT * FROM capitulo WHERE lista_id = ?1 AND usuario_id = ?2 AND cancion_id = ?3", nativeQuery = true)
	public Capitulo findById(int i, String s, int c);
	@Query(value = "SELECT * FROM capitulo WHERE usuario_id = ?1", nativeQuery = true)
	public Iterable<Capitulo> getByUser(String s);
	@Query(value = "SELECT * FROM capitulo WHERE lista_id = ?1 AND usuario_id = ?2", nativeQuery = true)
	public Capitulo[] listSongsInAlbumUser(int i, String s);
	@Query(value = "SELECT * FROM capitulo WHERE lista_id = ?1 AND usuario_id = ?2", nativeQuery = true)
	public Iterable<Capitulo> getListFromAlbum(int i, String s);
	@Query(value = "SELECT * FROM capitulo WHERE nombre LIKE %?1%", nativeQuery = true)
